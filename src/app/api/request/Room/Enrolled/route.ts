  ...
  try {
    ...
    if (!studentId || !userName) {
      return NextResponse.json(
        { error: "Student ID and username are required" },
        { status: 400 }
      );
    }
    // Get enrolled room for this student (1 student = 1 room)
    const enrolled = await client.query(
      `SELECT start_date, end_date, enrolled_code FROM student_enrolled WHERE student_id = $1 AND user_name = $2`,
      [studentId, userName]
    );
    ...
    return NextResponse.json({
      startdate: enrolled.rows[0].start_date,
      enddate: enrolled.rows[0].end_date,
